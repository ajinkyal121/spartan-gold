 ...
 * to a previous block.
 */
module.exports = class SmartBlock extends Block {
  /**
  ...
  constructor(
    ...
    // Storing smart contracts mapped to hash
  ...
  addTransaction(tx, client) {
    ...
    switch (tx.data.type) {
      ...
      case "ContractInvocation":
        let intrepreter = new SmartInterpreter(tx.data.address, this.balances);
        ...
        if (!tx.data.call) {
          throw new Error("The call to the smart contract not specified in the transaction.");
        }

        // Call interpreter to perform run smart contract operation
        intrepreter.smartIntrepreter(this.contractMap.get(tx.data.scriptHash) + tx.data.call);
        break;
